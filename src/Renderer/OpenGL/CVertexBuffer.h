...
{
public:
	CVertexBuffer(IRenderer *renderer = nullptr, const ECVertexComponent *components = nullptr, unsigned int componentCount = 0, const void *data = nullptr, unsigned int size = 0, bool dynamic = false) : m_initialized(false), m_indexBuffer(nullptr) { Initialize(renderer, components, componentCount, data, size, dynamic); }
	~CVertexBuffer();
